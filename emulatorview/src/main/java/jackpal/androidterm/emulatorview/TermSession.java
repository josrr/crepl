...
//import android.util.Log; /* added by José Ronquillo RIvera 2015-08-13 11:34 */
/**
    ...
    public TermSession(final boolean exitOnEOF) {
        ...
        mReaderThread = new Thread() {
            ...
            public void run() {
                try {
                    while(true) {
			/** added by José Ronquillo RIvera 2015-08-13 11:43 **/
			//Log.w("jackpal", "before mTermIn.read");
			/** **/
                        int read = mTermIn.read(mBuffer);
			/** added by José Ronquillo RIvera 2015-08-13 11:34 **/
			//Log.w("jackpal", "read: " + read + "; " + mBuffer);
			/** **/
                        if (read == -1) {
                            // EOF -- process exited
                            break;
